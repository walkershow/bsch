...
def get_iqy_valid_area(vm_id):
	...
	AND user_type = 11 
	AND STATUS = 1 
	AND mobile_no IS NOT NULL """.format(
        g_serverid, vm_id
    )
    print sql
    res = dbutil.select_sql(sql)
...
def get_iqyatv_valid_area(vm_id):
    sql = """select area from vm_users where server_id={0} and vm_id={1}
            and user_type=11 and status=0 and mobile_no is not null""".format(
        g_serverid, vm_id
    )
    res = dbutil.select_sql(sql)
    if not res:
            ...
            try:
                ...
                with utils.SimpleFlock("/tmp/area/{0}.lock".format(area), 1):
                    ...
                    if not ret:
                        ...
                        return
                    ret, task_id = g_taskallot.allot_by_priority(vm_id, area, None, 12)
...
def iqyall_business(vm_id):
    ...
    if not res:
        ...
        if count < g_pb:
            try:
                ret, task_id = g_taskallot.allot_without_dial(vm_id, 13)
                print "===== get task ======:", task_id
                if not ret or task_id is None:
                    logger.warn(
                        utils.auto_encoding(
            ...
            try:
                ...
                with utils.SimpleFlock("/tmp/area/{0}.lock".format(area), 1):
                    ...
                    if not ret:
                        logger.info("area:%s 不可用", area)
                        return
                    ret, task_id = g_taskallot.allot_by_priority(vm_id, area, None, 11)
                    print "===== get task ======:", task_id
                    if not ret or task_id is None:
                        logger.warn(
                            utils.auto_encoding(
        ...
        if count < g_pb:
            ...
            try:
                ...
                with utils.SimpleFlock("/tmp/area/{0}.lock".format(area), 1):
                    ...
                    ret, task_id = g_taskallot.allot_by_priority(vm_id, area, int(sgid))
                    print "===== get task ======:", task_id
                    if not ret or task_id is None:
                        logger.warn(
                            utils.auto_encoding(
