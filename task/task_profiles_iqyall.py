...
class TaskProfile_IQYALL(TaskProfile):
    def get_inited_profiles(self, vm_id, tty, uty, area):
        sql = (
            "select profile_id from vm_users "
            "where server_id={0} and vm_id={1} and user_type={2} and "
            "terminal_type={3} ".format(self.server_id, vm_id, uty, tty)
        )
        print sql
    ...
    def get_used_profiles(self, vm_id, user_type, terminal_type, task_group_id, area):
        """当天该任务组已使用的用户"""
        """观看类任务"""
        profiles = []
        sql = """select cur_profile_id from vm_cur_task where
        server_id=%d and vm_id=%d and
        user_type in(11,13) and terminal_type=%d  and
        status>3 and UNIX_TIMESTAMP( NOW( ) ) - UNIX_TIMESTAMP( start_time ) < 21600""" % (
            self.server_id,
            vm_id,
            terminal_type,
        )
    ...
    def get_using_profiles(self, vm_id, user_type, terminal_type, task_group_id, area):
        profiles = []
        sql = """select cur_profile_id from vm_cur_task where
        server_id=%d and vm_id=%d and
        user_type in(11,12,13) and terminal_type=%d and 
        status in(-1,1,2)""" % (
            self.server_id,
            vm_id,
            terminal_type,
    ...
    def get_task_usable_profiles(
        self, vm_id, user_type, terminal_type, task_group_id, area
    ):
        if user_type == 13:
            user_type = 11
    ...
    def set_cur_task_profile(self, vm_id, task_id, task_group_id, day, area):
        ...
        randtime = 1
        profile_id = self.get_task_usable_profiles(
            vm_id, r["user_type"], r["terminal_type"], task_group_id, area
        )
        if not profile_id:
            return False
