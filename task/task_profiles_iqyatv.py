...
class TaskProfile_IQYATV(TaskProfile):
    ...
    def get_inited_profiles(self, vm_id, tty, uty, area):
        sql=    "select profile_id from vm_users "\
            ...
            "terminal_type={3} and status=0 and mobile_no is not null " \
            "and area={4}"\
            .format( self.server_id, vm_id, uty, tty, area)
        self.logger.info("iqy active:"+sql)
    ...
    def get_used_profiles(self, vm_id, user_type, terminal_type, task_group_id,
        ...
        user_type in(11,12,13) and terminal_type=%d and task_group_id=%d and area=%s and
